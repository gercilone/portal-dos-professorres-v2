                    ...
                    todayScheds.map((sched) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left p-4 rounded-2xl border transition duration-200 cursor-pointer flex items-center justify-between gap-3 group relative overflow-hidden shadow-sm ${schoolColors.bg} ${schoolColors.border} hover:bg-opacity-90`}
                        >
                          <div className={`absolute top-0 left-0 w-1.5 h-full ${schoolColors.accentColor}`}></div>
                          <div className="space-y-1.5 w-full min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="font-bold text-zinc-100 text-xs bg-zinc-950/80 px-2 py-0.5 rounded border border-zinc-850">
                                {sched.timeSlot}
                              </span>
                              <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md border ${schoolColors.badgeBg} ${schoolColors.badgeText} ${schoolColors.badgeBorder}`}>
                          ...
                          <div className="bg-zinc-950 p-2 rounded-xl border border-zinc-800 group-hover:bg-zinc-900 transition-all shrink-0">
                      ...
                      .map((sched) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => handleSelectSchedule(sched)}
                            className={`w-full text-left p-3.5 rounded-xl border transition duration-150 cursor-pointer flex items-center justify-between gap-3 group ${schoolColors.bg} ${schoolColors.border} hover:bg-opacity-90`}
                          >
                            <div className="space-y-1.5 w-full min-w-0">
                              <div className="flex flex-wrap items-center gap-1.5">
                                <span className="font-extrabold text-[10px] bg-zinc-950 px-2 py-0.5 rounded border border-zinc-800 text-blue-400 font-sans">
                                  {getDayLabel(sched.dayOfWeek).split('-')[0]}
                                </span>
                                <span className="font-bold text-zinc-100 text-xs font-mono">
                                  {sched.timeSlot}
                                </span>
                                <span className={`text-[9px] font-extrabold uppercase px-1.5 py-0.2 rounded border ${schoolColors.badgeBg} ${schoolColors.badgeText} ${schoolColors.badgeBorder}`}>
                                  {sch?.name || 'Sem Escola'}
                                </span>
                              </div>
                              <div className="flex items-baseline gap-1.5 flex-wrap pt-0.5">
                            ...
                            <div className="bg-zinc-950 p-1.5 rounded-lg border border-zinc-850 group-hover:bg-zinc-900 transition-all shrink-0">
